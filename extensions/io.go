...
func ImportIO(env *glisp.Environment) {
	env.AddFunctionByConstructor("io/read-file", ReadFile)
	env.AddFunctionByConstructor("io/write-file", WriteFile)
	env.AddFunctionByConstructor("io/file-exist?", ExistFile)
	env.AddFunctionByConstructor("io/remove-file", RemoveFile)
}

func ReadFile(name string) glisp.UserFunction {
...
func WriteFile(name string) glisp.UserFunction {
	return func(env *glisp.Environment, args []glisp.Sexp) (glisp.Sexp, error) {
		if len(args) != 2 {
			return glisp.SexpNull, fmt.Errorf(`%s expect 1 argument but got %v`, name, len(args))
		}
...
func RemoveFile(name string) glisp.UserFunction {
	return func(env *glisp.Environment, args []glisp.Sexp) (glisp.Sexp, error) {
		if len(args) != 1 {
			return glisp.SexpNull, fmt.Errorf(`%s expect 1 argument but got %v`, name, len(args))
		}
...
func ExistFile(name string) glisp.UserFunction {
	return func(env *glisp.Environment, args []glisp.Sexp) (glisp.Sexp, error) {
		if len(args) != 1 {
			return glisp.SexpNull, fmt.Errorf(`%s expect 1 argument but got %v`, name, len(args))
		}
